...
{
    int* nums = get_dynamic_memory(num);//get memory
    //use memory
    for(int i =0; i<num; i++)
    {
        cout<<nums[i]<<"\n";
...
{
   //create memory
   shared_ptr<int[]> nums(get_dynamic_memory(num), delete_dynamic_memory); //shared ptr uses delete_dynamic memory

   //use memory
    nums[0] = 10;
    cout<<nums[0]<<"\n";

   //do not have to worry about calling delete; shared_ptr deletes for us
